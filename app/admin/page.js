...
export default function AdminPage() {
  ...
  async function handleCopyToCity() {
    if (!copyTarget || !copyDestCity) return
    setSaving(true)
    const totalStock = Object.values(copyTarget.flavor_stock || {}).reduce((a, b) => a + (parseInt(b) || 0), 0)
    const payload = {
      name: copyTarget.name,
      price: copyTarget.price,
      promo_price: copyTarget.promo_price || null,
      on_sale: copyTarget.on_sale,
      stock_qty: totalStock,
      image_url: copyTarget.image_url || null,
      flavors: copyTarget.flavors || [],
      flavor_stock: copyTarget.flavor_stock || {},
      cities: [copyDestCity],
    }
    const { error } = await supabase.from('pods').insert(payload)
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <div className="flex gap-2">
                            <button
                              onClick={() => { setCopyTarget(pod); setCopyDestCity('') }}
                              className="w-8 h-8 rounded-xl flex items-center justify-center text-white/30 hover:text-emerald-400 border border-white/[0.08] hover:border-emerald-500/40 transition-all"
                              title="Exportar para outra cidade"
                            ><I.Copy /></button>
                            <button
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <p>👤 <span className="text-white/40">Vendido Por:</span></p>
                      </div>
                    </div>
                  )
                })}
        ...
        {copyTarget && (
          <div>
            <p className="text-white/50 text-sm mb-1">
              Copiar <span className="text-white font-semibold">"{copyTarget.name}"</span> para:
            </p>
            <p className="text-white/25 text-xs mb-5">
              Será criada uma cópia independente na cidade destino, com estoque e preço próprios.
            </p>
            <div className="space-y-2 mb-6">
              {cities.filter(c => c !== activeCity).map(city => (
                <button
                  key={city}
                  onClick={() => setCopyDestCity(city)}
                  className="w-full text-left px-4 py-3 rounded-xl text-sm font-semibold transition-all"
                  style={{
                    background: copyDestCity === city ? 'rgba(16,185,129,0.12)' : 'rgba(255,255,255,0.04)',
                    border: copyDestCity === city ? '1px solid rgba(16,185,129,0.45)' : '1px solid rgba(255,255,255,0.08)',
                    color: copyDestCity === city ? '#6ee7b7' : 'rgba(255,255,255,0.5)',
                  }}
                >
                  📍 {city}
                </button>
            ...
            <div className="flex gap-3">
              ...
              >Cancelar</button>
              <button
                onClick={handleCopyToCity}
                disabled={!copyDestCity || saving}
